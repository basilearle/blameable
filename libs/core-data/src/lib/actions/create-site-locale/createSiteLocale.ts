...
export async function createSiteLocale(
  ...
  if (!siteTranslations.translations) {
    throw new Error('listSiteLocales: site locales not found or user does not have access');
  }
  ...
  if (!updatedSiteTranslations.translations) {
    throw new Error('listSiteLocales: site locales not found after update');
  }
